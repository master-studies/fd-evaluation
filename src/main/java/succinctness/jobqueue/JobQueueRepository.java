...
public interface JobQueueRepository extends JpaRepository<JobQueue, UUID> {
    ...
    @Query(value = "SELECT TOP (1) * FROM JobQueue WITH (ROWLOCK, READPAST, UPDLOCK) WHERE JobStatus = 'NEW' AND ServiceType = 'SUCCINCTNESS' ORDER BY CreatedAt", nativeQuery = true)
